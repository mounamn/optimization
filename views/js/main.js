...
var resizePizzas = function(size) {
    ...
    function changeSliderLabel(size) {
        switch (size) {
            case "1":
                document.getElementById("pizzaSize").innerHTML = "Small"; //using getElementById instead of querySelector to rapidly access
                return;
            case "2":
                document.getElementById("pizzaSize").innerHTML = "Medium"; //using getElementById instead of querySelector to rapidly access
                return;
            case "3":
                document.getElementById("pizzaSize").innerHTML = "Large"; //using getElementById instead of querySelector to rapidly access
                return;
            default:
    ...
    function determineDx(elem, size) {
        ...
        var windowWidth = document.getElementById("randomPizzas").offsetWidth; //using getElementById instead of querySelector to rapidly access
        ...
        function sizeSwitcher(size) {
            switch (size) {
                case "1":
                    return 0.25; // Changes the slider value to a percent width
                case "2":
                    return 0.33;
                case "3":
                    return 0.5;
    ...
    // Save the array in a lcoal variable outside the loop  
    var randomPizza = document.getElementsByClassName("randomPizzaContainer"); // Replace querySelectorAll by getElementsByClassName
    var dx = determineDx(randomPizza[0], size);;
    var newwidth = (randomPizza[0].offsetWidth + dx) + 'px';
    // Replace all document.getElementsByClassName("randomPizzaContainer") by the variable randomPizza
    function changePizzaSizes(size) {
        for (var i = 0, len = randomPizza.length; i < len; i++) { // add the end value in the loop
...
var docfrag = document.createDocumentFragment(); //Create a fragment
for (var i = 2; i < 100; i++) {
    docfrag.appendChild(pizzaElementGenerator(i)); // Append the elements to the fragment
}
pizzasDiv.appendChild(docfrag); // Append the elements to the DOM tree
...
function updatePositions() {
    ...
    // Move variables outside the loop
    var items = document.getElementsByClassName('mover'); // Access specified DOM elements much faster 
    var up = (document.body.scrollTop / 1250);
    var j = 0; // relpace (i % 5) by this variable which returns the same results
    for (var i = 0, len = items.length; i < len; i++) { // add the end value in the loop
        items[i].style.left = items[i].basicLeft + 100 * (Math.sin(up + j)) + 'px';
        //items[i].style.transform = 'translateX('+ items[i].basicLeft + 100 * (Math.sin( up + (i % 5))) + 'px)';
        j++;
        if (j == 5) {
            j = 0;
        }
...
document.addEventListener('DOMContentLoaded', function() {
    // Calculate the number of pizzas dynamically
    var cols = Math.ceil(window.innerWidth / 256); // Calculate the number of columns
    var rows = Math.ceil(window.innerHeight / 256); // Calculate the number of rows
    var pizzasNum = cols * rows;
    var s = 256;
    var elem;
    var moving = document.getElementById("movingPizzas1"); // Replace querySelector by getElementById
    var docfrag = document.createDocumentFragment(); //Create a fragment

    for (var i = 0; i < pizzasNum; i++) { // Reduce the number of moving pizzas from 200 to 40
        elem = document.createElement('img');
        elem.className = 'mover';
        elem.src = "images/pizza.png";
        /* elem.style.height = "100px"; // Moved to style.css
        elem.style.width = "73.333px"; */
        elem.basicLeft = (i % cols) * s;
        elem.style.top = (Math.floor(i / cols) * s) + 'px';
        docfrag.appendChild(elem); // Append the elements to the fragment
    }
    moving.appendChild(docfrag); // Append the elements to the DOM tree
    updatePositions();
});